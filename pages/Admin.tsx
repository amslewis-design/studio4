...
export default function Admin() {
  ...
  const [remoteAsset, setRemoteAsset] = useState({ name: '', url: '' });
  const [isLinking, setIsLinking] = useState(false);
  const [assetError, setAssetError] = useState<string | null>(null);

  // Initialize Quill 2.0
  useEffect(() => {
    if (editorContainerRef.current && !quillRef.current && tab === 'posts') {
  ...
  const handleSavePost = async () => {
    ...
    if (!postTitle || !htmlContent || htmlContent === '<p><br></p>') return;
  ...
  const handleAIHelp = async () => {
    ...
    try {
      ...
      if (quillRef.current) {
        quillRef.current.root.innerHTML = result || '';
      }
    } finally {
